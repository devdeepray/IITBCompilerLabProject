...
struct FunctionTable
{
  ...
  VarDeclaration getVar(string var_name)
  {
    return strVarMap[var_name];
  }
  ...
  list<ValType> getArgTypeList()
  {
	...
	it != paramOrderVector.end();
	  ++it)
	{
	  result.push_back(strVarMap[*it].varType->type);
	}
    return result;
  }
  
  pair<string, list<ValType> > getSignature()
  {
    return pair<string, list<ValType> >(getName(), getArgTypeList());
  }
